...
for n in range(nMice):
    ...
    for m in range(nCellPerAnimal):  # cells per animal
        gssub2 = gridspec.GridSpecFromSubplotSpec(2, 2, subplot_spec=gs[nPanel], hspace=0.1, wspace=0.3)
        nFig = 0
        nCase = 0
        for i in range(0,2):
            for j in range(1,3):
                ...
                if j>i:
                    ...
                    speed1 = PSTHquantifyDict[mice[n]][m][speedCases[i]]
                    ...
                    dT = psthT[1]-psthT[0]
                    ccPSTH = crosscorr(dT,psth1,psth2,correlationRange=0.39)
                    ccSpeed = crosscorr(dT, speed1PSTHTime, speed2PSTHTime, correlationRange=0.39)
                    ax0.axhline(0, ls=':', c='0.6')
                    ax0.axvline(0, ls=':', c='0.6')
                    ax0.plot(ccPSTH[:,0],ccPSTH[:,1],c='C0',label='PSTH')
                    ax0.plot(ccSpeed[:, 0], ccSpeed[:, 1], c='C1', label='speed')
                    #if nFig != 6:
                    layoutOfPanel(ax0, xLabel=(labels[j] if nFig>1 else None),yLabel=(labels[i] if not nFig%2 else None),Leg=([2] if nFig==0 else None))
                    #else:
...
plt.savefig('cross-correlations_%s.pdf' % cellType)
